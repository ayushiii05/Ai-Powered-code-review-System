...
const QUICK_ACTIONS = [
  { label: 'Explain Architecture', prompt: 'Can you provide a detailed explanation of how this project is architected?', icon: FolderTree },
  { label: 'Where is the database logic?', prompt: 'Where does the database connection and querying happen in this project?', icon: SearchCode },
  { label: 'How are routes handled?', prompt: 'How is routing handled in this codebase?', icon: SearchCode },
...
const ProjectChatAssistant = ({ projectId, project }) => {
  ...
  const fetchChatHistory = async () => {
    try {
      const { data } = await api.get(`/project/${projectId}/chat`);
      if (data.success && data.data.length > 0) {
        setMessages(data.data);
      } else {
        setMessages([{ role: 'assistant', content: `Hello! I am your AI Project Assistant. I've analyzed **${project?.projectName || 'this project'}**. Ask me anything about the architecture, files, or logic!` }]);
      }
    } catch (error) {
  ...
  const handleSend = async (messageText = input) => {
    ...
    try {
      const { data } = await api.post(`/project/${projectId}/chat`, {
        message: messageText
      });
